...
export async function POST(request: NextRequest) {
    try {
        ...
        const existing = await pool
            .request()
            .query("SELECT CodJuego FROM Juego WHERE CodJuego LIKE 'JUE%'")

        const codigos = existing.recordset
            .map((row) => parseInt(row.CodJuego.trim().slice(3), 10))
            .filter((num) => !isNaN(num))

        const siguienteNumero = Math.max(...codigos, 0) + 1
        const nuevoCodJuego = `JUE${String(siguienteNumero).padStart(3, '0')}` // Ej: JUE001, JUE002, etc.

        await pool
            .request()
            .input('codjuego', sql.NChar(6), nuevoCodJuego)
            ...
            .input('Fecha', sql.Date, body.Fecha).query(`
        INSERT INTO Juego (CodJuego, Descripcion, Equipo1, Equipo2, Fecha) 
        VALUES (@CodJuego, @Descripcion, @Equipo1, @Equipo2, @Fecha)
      `)

        return NextResponse.json({ message: 'Juego creado exitosamente', CodJuego: nuevoCodJuego })
    } catch (error) {
