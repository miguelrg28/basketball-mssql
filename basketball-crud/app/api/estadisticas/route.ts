...
export async function POST(request: NextRequest) {
    try {
        ...
        const existing = await pool.request().query('SELECT CodEstadistica FROM Estadistica')

        const codigos = existing.recordset
            .map((row) => parseInt(row.CodEstadistica.trim(), 10))
            .filter((num) => !isNaN(num))

        const siguienteNumero = Math.max(...codigos, 0) + 1
        const nuevoCodEstadistica = String(siguienteNumero).padStart(2, '0') // Ej: 01, 02, ..., 10, etc.

        await pool
            .request()
            .input('CodEstadistica', sql.NChar(5), nuevoCodEstadistica)
            .input('Descripcion', sql.VarChar(25), body.Descripcion)
            .input('Valor', sql.TinyInt, body.Valor).query(`
      ...
      `)
        ...
        return NextResponse.json({
            ...
            CodEstadistica: nuevoCodEstadistica,
        })
    } catch (error) {
